...
docker pull becchi78/iac_devcontainer:v1.8
...
docker run -d --name iac-devcontainer \
  ...
  becchi78/iac_devcontainer:v1.8
```
...
docker build -t iac_devcontainer:v1.8 .
```
...
```bash
docker build --build-arg BUILDARCH=amd64 --build-arg AWSCLIARCH=x86_64 --build-arg SAMCLIARCH=x86_64 -t iac_devcontainer:v1.8 .
```

#### Mac (Apple Silicon) の場合

```bash
docker build --build-arg BUILDARCH=arm64 --build-arg AWSCLIARCH=aarch64 --build-arg SAMCLIARCH=arm64 -t iac_devcontainer:v1.8 .
```
...
- v1.8 2026/06/04 全ツール最新化、checkov/gh追加、ECS CLI削除（EOL）
